...
export const ReactToSolidBridge = ({
  ...
  const [portalDomElement, setPortalDomElement] = useState();

  const parentDomElement = useRef<HTMLDivElement>(null);

  const getSolidComponentRef = useRef(getSolidComponent);

  useEffect(() => {
    getSolidComponentRef.current = getSolidComponent;
  }, [getSolidComponent]);

  const solidComponentRef = useRef(solidComponent);

  useEffect(() => {
    solidComponentRef.current = solidComponent;
  }, [solidComponent]);
  ...
  const solidSignalsRef = useRef(initialSolidSignals);

  useEffect(() => {
    for (let prop in props?.values || props) {
      if (typeof props[prop] === "function") {
        if (!solidSignalsRef.current[prop]) {
          solidSignalsRef.current = {
            ...solidSignalsRef.current,
            [prop]: [(...args) => propsRef.current[prop](...args)],
          };
        }
      } else {
        if (!solidSignalsRef.current[prop]) {
          ...
          solidSignalsRef.current[prop][1](props[prop]);
        }
      }
    }

    if (!solidPropsRef.current) {
      solidPropsRef.current = Object.fromEntries(
        Object.entries(solidSignalsRef.current).map(([key, value]) => [
          key,
          value[0],
        ])
      );
  ...
  useEffect(() => {
    ...
    const getSolidChildren = () => [
      SolidToReactPortalElement({
        getChildElement: setPortalDomElement,
      }),
        ...
        get children() {
          if (solidComponentRef.current) {
            const proxy = new Proxy(
              ...
              },
              {
                get: (proxy, propertyName) =>
                  propertyName in solidPropsRef.current
                    ? solidPropsRef.current[propertyName]()
                    : Reflect.get(proxy, propertyName),
              }
            );
            ...
            return getSolidComponentRef.current({
              ...
              props: solidPropsRef.current,
            });
          }
        },
        mount: parentDomElement.current,
      });
  ...
  return (
    <div ref={parentDomElement}>
      <ReactToSolidBridgeContext.Provider value={providerValue}>
        {children &&
          portalDomElement &&
          createPortal(children, portalDomElement)}
