...
export default function YogaClassCard({
  ...
  const handleJoin = async (e) => {
    ...
    try {
      ...
      if (!attendanceResponse.data.allowed) {
        popup?.close();
        toast.error(attendanceResponse.data.message);
        return;
      }

      // STEP 2: Activate class session (generate or reuse token)
      const activateResponse = await Fetch({
        url: `/api/classes/${classDetails.zoom_class_id}/activate`,
        method: "POST",
      });

      const secureJoinUrl = activateResponse?.data?.join_url;

      if (!secureJoinUrl) {
        popup?.close();
        toast.error("Unable to generate join link");
        return;
      }

      if (popup) {
        popup.location.href = secureJoinUrl;
      } else {
        window.location.href = secureJoinUrl;
      }
