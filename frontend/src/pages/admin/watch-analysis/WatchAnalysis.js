...
function WatchAnalysis() {
	...
	// signup-per-month
	// const { data: signUpPerMonth } = useQuery({
	// 	queryKey: ["signUpPerMonth"],
	// 	queryFn: async () => {
	// 		const res = await Fetch({
	// 			url: "/user/admin-stats/signup-per-month",
	// 			method: "GET",
	// 			token: true,
	// 		});
	...
	// /user-plan/admin-stats/users-per-plan
	...
	// /customUserPlan/admin-stats/users-per-plan
	...
	// /transaction/admin-stats/revenue-per-month
	...
	// /transaction/admin-stats/current-month-revenue
				...
				{loading ? (
					...
					<div className="flex justify-center items-center h-full">
						<Card width="30">
							...
							<Button
								onClick={() => handleDownload(watchTimeCount)}>
								Download CSV
							</Button>
					...
					<Card>
						...
						<dd className="mt-2 flex items-baseline space-x-2.5">
							<span className="text-tremor-metric font-semibold text-tremor-content-strong dark:text-dark-tremor-content-strong">
								{(
									(currentMonthUsage?.watch_time || 0) / 3600
								).toFixed(2)}{" "}
								hours
							</span>
						</dd>
							...
							{currentMonthRevenue?.map((revenue) => {
								return (
									<span
										...
										{(
											parseFloat(revenue?.revenue) / 100
										).toFixed(2)}
									</span>
								);
							})}
