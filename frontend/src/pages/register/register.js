...
export default function Register({ switchForm }) {
  ...
  const handleStudentRegistration = async (e) => {
    ...
    formData.phone_no = formData?.phone_no.replace(/\s/g, "");
    if (!formData.phone_no.startsWith("+91")) {
      formData.phone_no = "+91" + formData.phone_no;
    }

    const [is_phone_valid, phone_error] = validatePhone(
      formData?.phone_no.replace(/\s/g, "")
    );
    ...
    try {
      ...
      if (response.ok) {
        notify("New User added successfully! Kindly login.");
  ...
  const handleInstituteRegistration = async (e) => {
    ...
    Fetch({
      ...
      .then((res) => {
        ...
        if (res && res.status === 200) {
          ...
          Fetch({
            ...
            .catch((err) => {
              console.log(err);
              notify("Error registering user: " + err?.response?.data?.error);
            });
        } else {
      ...
      .catch((err) => {
        console.log(err);
        notify("Error registering institute: " + err?.response?.data?.error);
      });
  };
    ...
    <div className="bg-white p-4 rounded-lg max-w-xl mx-auto">
      ...
      <Tabs initialValue="student" leftSpace="150px">
        ...
        <Tabs.Item label="Register as Student" value="student">
          <StudentRegisterForm handleSubmit={handleStudentRegistration} />
        </Tabs.Item>
      </Tabs>
      <hr />
