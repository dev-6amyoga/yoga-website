      ...
      if (data?.userPlan.length !== 0) {
        ...
        for (var i = 0; i !== data?.userPlan.length; i++) {
          if (data?.userPlan[i].validity_to < formattedDate) {
            if (
              data?.userPlan[i].current_status !== USER_PLAN_EXPIRED_BY_DATE &&
              data?.userPlan[i].current_status !== USER_PLAN_EXPIRED_BY_USAGE
            ) {
