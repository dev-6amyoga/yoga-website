...
import { Card, Divider, Button } from "@geist-ui/core";
...
export default function UpdateScreen() {
  ...
  useEffect(() => {
    if (token) {
      Fetch({
        url: "http://localhost:4000/update-request/get-update-request-by-token",
        method: "POST",
        data: {
          token: token,
        },
  ...
  useEffect(() => {
    const fetchData = async () => {
      Fetch({
        url: "http://localhost:4000/user/get-by-email-and-name",
        method: "POST",
        data: { name: updateRequest?.name, email: updateRequest?.old_email },
      })
        .then((res) => {
  ...
  const accepted = async () => {
    Fetch({
      url: "http://localhost:4000/user/update-email",
      method: "POST",
      data: {
        user_id: userId,
