			...
			try {
				...
				if (res.status === 200) {
					if (res.data.plans) {
						...
						const playlistsPromises = sortedPlans.map(
							async (plan) => {
						...
						const playlistsData =
							await Promise.all(playlistsPromises);
						const today1 = new Date();

						let activePlaylists = [];
						for (
							var i = 0;
							i !== playlistsData.flat().length;
							i++
						) {
							let planiD = playlistsData.flat()[i]._id;
							let playlistsInternal =
								playlistsData.flat()[i].playlists;
							...
							let startDate = new Date(
								matchingUserPlan.validity_from
							);
							let endDate = new Date(
								matchingUserPlan.validity_to
							);
							let daysSinceStart =
								Math.floor(
									(today1 - startDate) / (1000 * 60 * 60 * 24)
								) + 1;
							for (
								var j = 0;
								j !== playlistsInternal.length;
								j++
							) {
								let playlistId = Number(
									Object.keys(playlistsInternal[j])[0]
								);
								let active = Object.values(
									playlistsInternal[j]
								);
						...
						const uniqueActivePlaylists = [
							...new Set(
								activePlaylists.filter((p) => p !== null)
							),
						];
					...
					setAsanaDetails(
						Object.values(
							asanasDetails.reduce((acc, current) => {
								...
								const currentID =
									current?.id || current?.transition_id;
								// console.log(acc, currentID, current);
								const isTransition =
									current?.transition_id !== undefined
										? true
										: false;
								...
								if (
									acc[currentID] &&
									((isTransition &&
										acc[currentID]?.transition_id ===
											currentID) ||
										acc[currentID]?.id === currentID)
								) {
									acc[currentID].count =
										acc[currentID].count + 1;
								} else {
				...
				playlist.asana_ids.forEach((asana_id) => {
					if (typeof asana_id === "number") {
						...
						const transition = allTransitions.find(
							(transition) =>
								transition.transition_id === asana_id
						);
						if (transition) {
