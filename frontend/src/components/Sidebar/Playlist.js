    ...
    const fetchData = async () => {
      try {
        ...
        const playlistsData = await Promise.all(
          validPlans.map((plan) => fetchCustomPlanDetails(plan.custom_plan_id))
        );
        let activePlaylists = [];
        playlistsData.flat().forEach((plan) => {
          ...
          plan.playlists.forEach((playlist) => {
            const [activeFrom, activeTo] = Object.values(playlist)[0];
            if (daysSinceStart >= activeFrom && daysSinceStart <= activeTo) {
              activePlaylists.push(Number(Object.keys(playlist)[0]));
            }
          });
        });
