...
import {
  ...
  GetPlayreadyTokenData,
  GetWidevineTokenData,
} from "../../api/token.api";
...
function StreamStackItem({
  ...
  const [shakaOfflineStore] = useShakaOfflineStore((state) => [
  ...
  const saveDownloadTimestamp = useCallback(async (videoUrl, timestamp) => {
  ...
  const showAllRecords = useCallback(async (videoUrl) => {
    try {
      const db = await openDatabase();
  ...
  const loadVideo = useCallback(
    async (offlineSupported, isDrm, lastUpdated) => {
      ...
      try {
        ...
        // Redownload video if it's outdated
        if (
          savedTimestamp &&
          lastUpdated &&
  ...
  const handleDrmDownload = useCallback(
    async (videoUrl, videoTitle) => {
      try {
        let drmConfig = null;
        let error = null;
        ...
        switch (detectedBrowser) {
          ...
          case browser.opera:
            ...
            playerRef.current.player.configure({
              drm: {
                servers: {
                  "com.widevine.alpha": drmConfig.licenseAcquisitionUrl,
                },
              },
            });
            break;
    ...
    [saveDownloadTimestamp, shakaOfflineStore]
  );

  const handleNonDrmDownload = useCallback(
    async (videoUrl, videoTitle) => {
      try {
        const offlineVideo = await shakaOfflineStore.store(
          videoUrl,
  ...
  const handleOnlineVideoLoad = useCallback(async (videoUrl) => {
    const detectedBrowser = detectBrowser();
    ...
    switch (detectedBrowser) {
      ...
      case browser.opera:
        ...
        break;
      case browser.edge:
        ({ data: drmConfig, error } = await GetPlayreadyTokenData());
        playerRef.current.player.configure({
          drm: {
            servers: {
              "com.microsoft.playready": drmConfig.licenseAcquisitionUrl,
            },
          },
        });
        break;
  ...
  const handleOfflineVideoLoad = useCallback(
    async (offlineUri) => {
  ...
  const setupEventListeners = useCallback(() => {
    playerRef.current.player.addEventListener("loading", handlePlayerLoading);
