    ...
    (e) => {
      ...
      if (isActiveRef.current) {
        if (e.newstate === "buffering") {
          console.log("SET VIDEO STATE LOADING : handleVideoStateChange");
  ...
  const playerInit = useCallback(
    (ref) => {
      if (ref !== null) {
        ...
        if (ref.videoElement) {
          playerRef.current.videoElement.addEventListener("seeking", (e) => {
            console.log("Seeking...");
            setVideoState(STATE_VIDEO_LOADING);
          });
          playerRef.current.videoElement.addEventListener("seeked", (e) => {
            console.log("Seeked...");
            setVideoState(STATE_VIDEO_PLAY);
          });
