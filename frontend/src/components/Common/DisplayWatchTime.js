      ...
      if (timerInterval.current === null) {
        timerInterval.current = setInterval(() => {
          ...
          if (hasStarted) {
            ...
            setTimeArray(convertSecondsToHMS(Math.floor(distance / 1000)));
          }
        }, 1000);
      }
      return () => {
      ...
      {endTs !== null ? (
        <>
          <p className="flex flex-col items-center gap-2">
            <span className="font-mono text-5xl">{timeArray.hours}</span>
            <span>Hrs.</span>
          </p>
          <p className="flex flex-col items-center gap-2">
            <span className="font-mono text-5xl">{timeArray.minutes}</span>
            <span>Min.</span>
          </p>
          <p className="flex flex-col items-center gap-2">
            <span className="font-mono text-5xl">{timeArray.seconds}</span>
            <span>Sec.</span>
          </p>
        </>
      ) : (
