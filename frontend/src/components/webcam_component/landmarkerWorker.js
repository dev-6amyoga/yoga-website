...
const detectVrikshasanaLeft = (landmarks) => {
  ...
  if (base_score === 10) {
    if (leftHeelY === rightAnkleY || Math.abs(leftHeelY - rightAnkleY) < 0.1) {
      leg_score = 20;
...
const detectVrikshasanaRight = (landmarks) => {
  ...
  if (base_score === 10) {
    if (leftHeelY === leftAnkleY || Math.abs(leftHeelY - leftAnkleY) < 0.1) {
      leg_score = 20;
      legPositionMessage = "Try and lift your leg to the groin!";
...
self.onmessage = async (event) => {
  ...
  if (data.type === "init") {
    ...
    drawingUtils = new module.DrawingUtils(offscreenCanvas.getContext("2d"));
