...
router.post("/check-plan-mapping", authenticateToken, async (req, res) => {
  const { plan_id, custom_plan, coupon_name } = req.body;
  const { user_id } = req.user;
  console.log(plan_id, custom_plan, coupon_name);
  if (!plan_id || !coupon_name || !user_id || !custom_plan) {
    return res
  ...
  try {
    ...
    const transactions = await Transaction.findAll({
      where: {
        user_id: user_id,
        discount_coupon_id: discount_coupon_applicable_plan.discount_coupon_id,
