...
router.post('/check-plan-mapping', authenticateToken, async (req, res) => {
  ...
  if (is_custom_plan && typeof plan_id !== 'string') {
    return res.status(HTTP_BAD_REQUEST).json({ message: 'Invalid plan_id' })
  ...
  try {
    ...
    if (!is_custom_plan) {
      ...
      const discount_coupon_applicable_custom_plan =
        await DiscountCouponApplicableCustomPlan.findOne({
          discount_coupon_id: discount_coupon.get('discount_coupon_id'),
    ...
    if (transactions.length > 0) {
      return res.status(HTTP_BAD_REQUEST).json({ error: 'Coupon already used' })
    }
    ...
    if (!is_custom_plan) {
      ...
      const discount_coupon_applicable_custom_plan =
        await DiscountCouponApplicableCustomPlan.findOne(
          {
            discount_coupon_id: discount_coupon_id,
            custom_plan_id: plan_id.toString(),
          },
          {},
          options
        )
...
router.post('/remove-plan-mapping', async (req, res) => {
  ...
  if (!is_custom_plan) {
    ...
    try {
      const n_deleted = await DiscountCouponApplicablePlan.destroy(
        {
          where: {
            discount_coupon_id: discount_coupon_id,
            plan_id: plan_id,
    ...
    try {
      const n_deleted = await DiscountCouponApplicableCustomPlan.deleteOne(
        {
          discount_coupon_id: discount_coupon_id,
          custom_plan_id: plan_id.toString(),
        },
...
router.post('/get-all', async (req, res) => {
  try {
    ...
    let applicable_custom_plans =
      await DiscountCouponApplicableCustomPlan.aggregate([
        {
          $lookup: {
            from: 'custom_plan',
    ...
    let discount_coupons = dcs.map((dc) => {
      return {
        ...
        discount_coupon_applicable_custom_plans: applicable_custom_plans.filter(
          (ap) => ap.discount_coupon_id === dc.discount_coupon_id
        ),
      }
    })
