...
router.post("/create-email-update-verification", async (req, res) => {
  const { name, email, updated_email } = req.body;
  if (!email || !name || !updated_email) {
    return res.status(HTTP_BAD_REQUEST).json({
      message: "Missing required fields",
    });
  }
  ...
  try {
    ...
    <p>We received a request to update your Email ID to: ${updated_email}.</p>
