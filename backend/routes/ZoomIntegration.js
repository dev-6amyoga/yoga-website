...
router.get('/api/classes/:id/activate', async (req, res) => {
  const classData = await ZoomClassModel.findByPk(req.params.id)
  if (!classData) return res.status(404).json({ error: 'Not found' })

  const now = moment().tz('Asia/Kolkata')

  if (
    classData.join_token &&
    classData.join_token_expiry &&
    moment(classData.join_token_expiry).isAfter(now)
  ) {
    return res.json({
      join_url: `${process.env.BACKEND_DOMAIN}/zoom/join/${classData.join_token}`,
    })
  }

  // Otherwise create new token (first join of session)
  const newPassword = await rotatePMIPassword()
  const secureToken = crypto.randomBytes(32).toString('hex')

  let endTime
  if (classData.class_type === 'one_time') {
    endTime = moment(classData.end_time)
  } else {
    endTime = moment(
      `${now.format('YYYY-MM-DD')} ${classData.recurring_end_time}`
    ).tz('Asia/Kolkata')
  }

  await classData.update({
    zoom_meeting_password: newPassword,
    join_token: secureToken,
    join_token_expiry: endTime.toDate(),
  })

  return res.json({
    join_url: `${process.env.BACKEND_DOMAIN}/zoom/join/${secureToken}`,
  })
})

router.get('/join/:token', async (req, res) => {
  try {
    const classData = await ZoomClassModel.findOne({
      where: { join_token: req.params.token },
    })

    if (!classData) return res.status(404).send('Invalid or expired link')

    if (moment().isAfter(classData.join_token_expiry))
      return res.status(403).send('Link expired')

    // const zoomUrl = `https://us02web.zoom.us/j/${classData.zoom_meeting_id}?pwd=${classData.zoom_meeting_password}`

    // return res.redirect(zoomUrl)
    const zoomUrl = `zoommtg://zoom.us/join?confno=${classData.zoom_meeting_id}&pwd=${classData.zoom_meeting_password}`
