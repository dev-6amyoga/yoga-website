  ...
  try {
    ...
    if (expired) {
      ...
      if (stagedPlan) {
        ...
        await WatchTimeQuota.create(
          [
            {
              user_plan_id: String(stagedPlan.get('user_plan_id')),
    ...
    if (activeCustomUserPlans.length > 0) {
      ...
      for (let i = 0; i < activeCustomUserPlans.length; i++) {
        ...
        if (processedCustomPlanIds.includes(customUserPlan.custom_plan_id)) {
          continue
        }
        ...
        if (expired) {
          ...
          // promote staged to active if any
          const stagedCustomUserPlans = await CustomUserPlan.aggregate(filter, {
            session: mt,
          })
          ...
          stagedCustomUserPlan.set('validity_to', validity_to.toISOString())
            ...
            [
              {
                ...
                quota:
                  stagedCustomUserPlan.get('plan').get('watchHours') * 60 * 60,
              },
            ],
            { session: mt }
          )
...
const GetPlanForWatchQuotaDeduction = async (
  ...
  try {
    ...
    if (playlist_id) {
      ...
      console.log('active custom plans : ', activeCustomPlans.length)

      // if active plans found, return the first one

      // if active custom plans found, return the first one
      if (activeCustomPlans.length > 0) {
        // check watch time quota for active custom plan
        if (activeCustomPlans[0]?._id) {
          const wtq = await WatchTimeQuota.findOne({
            user_plan_id: activeCustomPlans[0]._id.toString(),
          })
          ...
          if (!wtq) {
            await WatchTimeQuota.create(
              {
                user_plan_id: activeCustomPlans[0]._id.toString(),
                quota: activeCustomPlans[0].plan.watchHours * 60 * 60,
              },
              { session: mt }
            )
          }
          ...
          console.log(
            'found watch time quota for :',
            activeCustomPlans[0].custom_plan_id
          )
        }
      ...
      if (activePlans.length > 0) {
        if (activePlans[0]?.get('user_plan_id')) {
          const wtq = await WatchTimeQuota.findOne({
            user_plan_id: String(activePlans[0].get('user_plan_id')),
          })

          if (!wtq) {
            console.log(
              'no watch time quota for :',
              activePlans[0].get('user_plan_id')
            )

            await WatchTimeQuota.create(
              {
                user_plan_id: String(
                  activePlans[0].getDataValue('user_plan_id')
                ),
                quota: activePlans[0]?.Plan.watch_time_limit,
              },
              { session: mt }
            )

            console.log(
              'created watch time quota for :',
              activePlans[0].user_plan_id
            )
          }
